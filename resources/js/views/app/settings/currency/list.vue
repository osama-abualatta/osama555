    ...
    <div class="main-content">
        ...
        <b-card class="wrapper">
            <g-list
                resource="currency"
                :endPoint="
                    route('settings.currencies.fetch')
                "
            />
            ...
            <!--<b-dropdown-item>-->
            <!--<router-link :to="'/settings/currencies/edit/'+item.id">-->
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                { width: "15%", label: "Currency", name: "icon" },
                {
                    width: "15%",
                    label: "Status",
                    name: "is_active",
    ...
    created() {
        this.source = routes.settings.currencies.fetch();
    },
    methods: {
        sortBy(key) {
            this.sortKey = key;
        ...
        changeStatus(item) {
            ...
            makeRequest(
                "PATCH",
                routes.settings.currencies.activate(item.id),
                {
                    is_active: !item.is_active
                },
                function(response) {
                    if (response.data.status === 200) {
                        self.$refs.datatable.fetch(
                            self.$refs.datatable.pagination.currentPage
                        );
                    }
                },
                function(xhr) {}
        ...
        deleteItem(item) {
            ...
            makeRequest(
                "DELETE",
                routes.settings.currencies.delete(item.id),
                {
                    is_active: !item.is_active
                },
                function(response) {
